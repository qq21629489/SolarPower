...
def Predict(data, last_date):
    ...
    date = datetime.datetime(
        2018, last_date[0], last_date[1], last_date[2], 0, 0)
...
def config():
    ...
    parser.add_argument("--bidresult", default="./sample_data/bidresult.csv",
                        help="input the bids result path")
...
def decision(resultG, resultC, date):
    result = []
    for i in range(0, len(resultG)):
        date = date + datetime.timedelta(hours=1)
        dif = resultC[i] - resultG[i]
        if (dif > 0):  # 消耗 > 產量
            # print(dif)
            for j in range(int(dif)):
                result.append([str(date), "buy", 2.5, 3])
            # print(result)
        elif (dif < 0):  # 消耗 < 產量
            # print(dif)
            for j in range(int(dif)):
                result.append([str(date), "buy", 2.5, 3])
            result.append([date, "sell", 3, 5])
            # print(result)
    return result
